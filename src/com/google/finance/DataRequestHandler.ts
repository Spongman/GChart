...
import { ChartEventTypes, ChartEvent } from './ChartEvent';
...
import { Graphics } from '../../../flash/display/Graphics';
	...
	export class DataRequestHandler {
		...
		private readonly urlLoader = new URLLoader();
		...
		constructor(private readonly dataManager: DataManager, private readonly url: string, private readonly event: ChartEvent) {
			...
			if (url) {
				...
				this.urlLoader.load(new URLRequest(/*"https:// finance.google.com" +*/ url));
			}
		}
		...
		private completeHandler(event: Event) {
			// const _loc2_ = this.event.target as URLLoader;
			clearInterval(this.intervalId);
