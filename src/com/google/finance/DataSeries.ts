...
import { VolumeIndicatorPoint, IndicatorPoint } from './indicator/IndicatorPoint';
	...
	export class DataSeries {
		...
		printFridays() {
			for (const friday of this.fridays) {
				if (this.units[friday]) {
					...
					console.log(this.points[friday].point + " " + (this.points[friday] as VolumeIndicatorPoint).volume);
				}
			}
		}
		...
		compareRefPointAndRelativeMinute(indicatorPoint: IndicatorPoint, param2: number): number {
		...
		getSessionForMinute(param1: number): MarketSessionPair | null {
			...
			return this.dataSessions.getClosestEarlierIntervalForValue(param1) as MarketSessionPair;
		}
		...
		private getDataSession(name: string): MarketSessionPair | null {
			const numSessions = this.dataSessions.length();
			for (let sessionIndex = 0; sessionIndex < numSessions; sessionIndex++) {
				const pair = this.dataSessions.getIntervalAt(sessionIndex) as MarketSessionPair;
				if (pair.name === name) {
					return pair;
				}
			}
