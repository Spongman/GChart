namespace com.google.finance
{
	...
	export class GVizFormatConverter
	{
		...
		{
			let _loc2_ = !!param1 ? param1[GVizFormatConverter.COLS_PROPERTY_NAME] : null;
		...
		static convertGVizData(param1:any, param2: number, param3: DataSeries, param4: Indicator) 
		{
			...
			let _loc5_ = param3.getPointsInIntervalArray(param2);
			if (!_loc5_ || _loc5_.length === 0)
				return;

			let _loc6_ = param1[GVizFormatConverter.ROWS_PROPERTY_NAME];
			let _loc7_: DataSeries[] = [];
			let _loc8_= 1;
			while (_loc8_ < param1[GVizFormatConverter.COLS_PROPERTY_NAME].length)
			{
			...
			{
				let _loc9_ = _loc6_[_loc8_][GVizFormatConverter.COL_PROPERTY_NAME][0][GVizFormatConverter.VALUE_PROPERTY_NAME];
				_loc10_ = Date.parse(_loc9_.toString());
				_loc11_ = param3.getTimestampIndex(_loc10_, _loc5_);
				let _loc12_ = _loc5_[_loc11_];
				_loc13_ = 1;
				while (_loc13_ < _loc6_[_loc8_][GVizFormatConverter.COL_PROPERTY_NAME].length)
				{
					_loc14_ = _loc6_[_loc8_][GVizFormatConverter.COL_PROPERTY_NAME][_loc13_][GVizFormatConverter.VALUE_PROPERTY_NAME];
