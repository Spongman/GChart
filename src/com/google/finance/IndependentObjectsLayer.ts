namespace com.google.finance
{
	...
	export class IndependentObjectsLayer extends AbstractLayer<ViewPoint>
	{
		...
		constructor(param1: ViewPoint, param2: DataSource)
		{
			...
			this.movies = [];
			this.activeMovies = 0;
			this.positioning = IndependentObjectsLayer.POSITION_CHART;
		...
		renderLayer(context: Context) 
		{
			...
			let _loc4_ = this.viewPoint.getDetailLevel();
			this.activeMovies = 0;
			let _loc5_ = this.getFirstVisibleObject(_loc2_, _loc3_, context);
			if (_loc5_ === -1)
			{
				this.resetCanvas();
				return;
			}
			let _loc6_ = this.getLastVisibleObject(_loc2_, _loc3_, context);
			...
			while (_loc9_ <= _loc6_)
			{
				let _loc10_ = this.getPosition(_loc2_[_loc9_], _loc3_, _loc4_, context);
				let _loc11_ = this.putObject(_loc2_[_loc9_], _loc10_);
				...
				if (this.avoidObj && context[this.avoidObj])
				{
					...
					{
						let _loc12_ = context[this.avoidObj][_loc8_];
