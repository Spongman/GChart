...
export class LayersManager {
	...
	private readonly takenColors: Array<{ quote: string, color: number }> = [];
	...
	static getViewPointIndex(value: string, viewPoints: IViewPoint[], key: string): number {
		...
		while (index >= 0 && (viewPoints[index] as any)[key] !== value) {	// TODO:
			index--;
		}
		return index;
	}
		...
		switch (indicatorType) {
			...
			default:
				if (Const.INDICATOR_ENABLED) {
					...
					const indicatorLayer = this.displayManager.addLayer(indicatorClassName, param3, source, param2) as IndicatorLayer | null;
					if (indicatorLayer) {
	...
	private getLayerId(layer: AbstractLayer<ViewPoint> | LayerInfo): string {
		switch (layer.name) {
			case "SimpleMovingAverage":
				return layer.name + "-" + (layer as any).length;	// TODO
			case "IndependentObjectsLayer":
				return "Objects" + (layer as IndependentObjectsLayer).renderObj;
			case "IntervalBasedIndependentObjectsLayer":
				return "Objects" + (layer as IntervalBasedIndependentObjectsLayer).renderObj;
			default:
				return layer.name;
		}
	}
		...
		if (_loc3_ === -1) {
				...
				case "indicator":
					...
					for (const key of Object.keys(layerInfo)) {
						if (key !== "name" && key !== "vp" && key !== "arity" && key !== "type") {
							(newIndicatorLayerInfo as any)[key] = (layerInfo as any)[key];	// TODO
						}
					}
			...
			for (const key of Object.keys(layerInfo)) {
				if (key !== "name" && key !== "vp" && key !== "arity") {
					(layer as any)[key] = (layerInfo as any)[key];
					(newLayerInfo as any)[key] = (layerInfo as any)[key];
				}
			}
	...
	private addViewPoints(viewPoints: ViewPoint[], except: IViewPoint[], param3: number) {
		...
		for (const viewPoint of viewPoints) {
			if (LayersManager.getViewPointIndex(viewPoint.name, except, "name") === -1) {
				...
				const existingViewPoint = this.displayManager.getViewPoint(viewPoint.name) as ViewPoint;
	...
	resetLayersForNewQuote(dataSource: DataSource, style: string) {
		...
		for (const viewPoint of viewPoints) {
			if (viewPoint.name === Const.MAIN_VIEW_POINT_NAME) {
				...
				(viewPoint as SparklineViewPoint).replaceSparklineDataSource(dataSource);
			}
		}
