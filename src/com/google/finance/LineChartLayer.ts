...
export class LineChartLayer extends AbstractDrawingLayer<ViewPoint> {
	...
	protected highlightCanvas = new Sprite("highlightCanvas");
	...
	protected drawWeeklyLine(sprite: Sprite, param2: number, param3: number, viewPoint: ViewPoint, context: Context): number {
	...
	protected drawDayLine(sprite: Sprite, unitIndex: number, viewPoint: ViewPoint, param4: number, param5: number, context: Context): number {
		const dataSeries = this.getDataSeries();
		const units = dataSeries.units;
		...
		if (units[_loc17_].relativeMinutes === 0 && units[_loc17_].fake) {
			...
			const point = new Point(viewPoint.getXPos(units[_loc17_]), this.viewPoint.maxy);
	...
	protected drawLine(sprite: Sprite, unitIndex: number, param3: number, viewPoint: ViewPoint, context: Context): number {
		const dataSeries = this.getDataSeries();
		const units = dataSeries.units;
		const gr = sprite.graphics;
		gr.lineStyle(this.lineThickness, this.lineColor, this.lineVisibility);
	...
	renderLayer(context: Context) {
		...
		gr.lineStyle(0, 0, 0);
		const point = new Point(_loc6_, viewPoint.maxy);
	...
	protected drawLineToDataUnit(viewPoint: ViewPoint, sprite: Sprite, context: Context, dataUnit: DataUnit) {
