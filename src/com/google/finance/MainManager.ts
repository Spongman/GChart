...
import { QuoteTypes, AddStreamResults, Const, ChartDetailTypes, ScaleTypes, Intervals } from './Const';
import { DisplayManager } from './DisplayManager';
import { LayersManager } from './LayersManager';
import { DataManager } from './DataManager';
import { Stage, StageAlign, StageScaleMode } from '../../../flash/display/Stage';
import { DataSource } from './DataSource';
import { ChartEventPriorities, ChartEvent } from './ChartEvent';
...
export class MainManager extends Sprite {
		...
		constructor(stage: Stage) {
			...
			if (this.stage.stageWidth === 0) {
				this.waitSizeIntId = setInterval(Stage.bind(this.checkSize, this), 20);
			} else {
				this.init();
			}
		}
		...
		private parseSettingString(param1: string, param2: number): Array<string | null> | null {
			const parts: Array<string | null> | null = param1 ? param1.split(";") : null;
		...
		private init() {
			...
			this.stage.addEventListener(Events.RESIZE, Stage.bind(this.onResize, this));
		...
		private checkParameters() {
			...
			Const.VOLUME_PLUS_ENABLED = "true" === String(Utils.checkUndefined(MainManager.paramsObj.enableVolumePlus, Const.isZhLocale(DateTimeLocale.getLocale())));
			Const.CHART_TYPE_BUTTONS_ENABLED = "true" === String(Utils.checkUndefined(MainManager.paramsObj.enableChartTypeButtons, Const.isZhLocale(DateTimeLocale.getLocale())));
			Const.APPLY_CHINESE_STYLE_MACD = "true" === String(Utils.checkUndefined(MainManager.paramsObj.applyChineseStyleMacd, DateTimeLocale.getLocale() === Const.ZH_CN_LOCALE));
			...
			if (this.quote !== "" && MainManager.paramsObj.compareTo) {
				...
				for (let partIndex = 0; partIndex < _loc1_.length; partIndex++) {
					...
					if (_loc5_ !== "" && _loc5_ !== this.quote) {
						this.addCompareTo(_loc5_, _loc2_ && _loc2_[partIndex] || undefined, _loc3_ && _loc3_[partIndex] === "1" || undefined);
					}	// TODO
				}
			}
		}
		...
		htmlClicked(ticker: string, param2: number, param3?: string) {
			const _loc4_ = this.dataManager.selectObject(ticker, "newspin", param2, param3) as PinPoint;
