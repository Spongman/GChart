...
namespace com.google.finance
{
	...
	export class SpaceText extends flash.display.Sprite
	{
		...
		private negativeTextFormat: flash.text.TextFormat;
		...
		private grayText: flash.text.TextFormat;

		startDate: com.google.finance.DateTextField;

		private bg: flash.display.Sprite;

		private blackText: flash.text.TextFormat;

		private datesText: string;

		private positiveTextFormat: flash.text.TextFormat;
		...
		constructor(param1: com.google.finance.DisplayManager)
		{
			...
			this.blackText = new flash.text.TextFormat("Verdana", 9, 0);
			this.blackText.align = flash.text.TextFormatAlign.RIGHT;
			this.negativeTextFormat = new flash.text.TextFormat("Verdana", 9, Const.NEGATIVE_DIFFERENCE_COLOR);
			this.negativeTextFormat.align = flash.text.TextFormatAlign.RIGHT;
			this.positiveTextFormat = new flash.text.TextFormat("Verdana", 9, Const.POSITIVE_DIFFERENCE_COLOR);
			this.positiveTextFormat.align = flash.text.TextFormatAlign.RIGHT;
			this.grayText = new flash.text.TextFormat("Verdana", 9, 0x777777);
			this.grayText.align = flash.text.TextFormatAlign.RIGHT;
			let _loc2_ = Const.MOVIE_WIDTH - SpaceText.RIGHT_PADDING;
			let _loc3_ = Const.SPACE_HEIGHT / 2 - SpaceText.TEXT_HEIGHT / 2 + Const.INFO_TEXT_TOP_PADDING;
		...
		{
			let _loc2_ = param1.getLastNotVisibleDataUnit();
			let _loc3_ = param1.getLastDataUnit();
			let _loc4_ = param1.getBaseDataSource();
		...
		private setDateEntryState(param1: number) 
		{
			...
			let _loc2_ = this.dateEntryState === SpaceText.EDIT_DATES;
		...
		private clearComparisonInfo() 
		{
			...
			{
				let _loc2_ = this.infoDots[_loc1_];
		...
		{
			let _loc3_ = new flash.text.TextField();
		...
		private appendOhlcText(param1: string, param2: number, param3: number) 
		{
			...
			let _loc4_ = this.normalizePrice(param2);
			if (param3 !== -1)
			{
				if (param2 > param3)
				{
		...
		{
			let _loc1_ = this.displayManager.mainController.getButtonsWidth();
			let _loc2_ = 0;
		...
		{
			let _loc2_ = param1[SpaceText.POINT_STR];
			let _loc3_ = "";
			...
			this.datesTextFormats = [];
			let _loc4_ = this.getSinglePointDateFormat(_loc2_);
			let _loc5_ = _loc2_.exchangeDateInUTC;
		...
		{
			let _loc5_ = NaN;
			let _loc2_ = !!this.startDate.visible ? Number(this.startDate.width + 3) : 0;
			let _loc3_ = !!this.endDate.visible ? Number(this.endDate.width + 3) : 0;
			let _loc4_ = this.infoText.width + 2;
			...
			if (param1)
			{
				_loc5_ = Const.SPACE_HEIGHT / 2 - SpaceText.TEXT_HEIGHT / 2 + Const.INFO_TEXT_TOP_PADDING;
		...
		private positionComparisonInfoDots() 
		{
			...
			{
				let _loc3_ = this.infoDots[_loc2_];
		...
		private updateInfoText() 
		{
			...
			else
			{
				...
				{
					let _loc3_ = this.datesTextFormats[_loc2_];
		...
		{
			let _loc2_ = param1[SpaceText.POINTS_STR];
			this.returnText = "";
			this.updateInfoText();
			this.clearComparisonInfo();
		...
		private getReturnText(param1: DataUnit | null, param2: DataUnit | null, param3: DataSource | null): string
		{
			...
			let _loc6_ = Math.round(_loc5_ / param1.close * 10000) / 100;
			if (_loc5_ > 0)
				_loc4_ = _loc4_ + "+";
		...
		{
			let _loc4_ = new com.google.finance.DateTextField();
