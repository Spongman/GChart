...
export abstract class IndicatorLayer extends AbstractDrawingLayer<ViewPoint> {
		...
		protected indicatorNameText: TextField;

		indicator: Indicator;

		protected medianLineCanvas: Sprite;

		protected textOutCanvas: Sprite;

		protected localMedianValue = 0;

		protected localYScale = 0;

		protected indicatorValueTextArray: TextField[];
		...
		constructor(viewPoint: ViewPoint, dataSource: DataSource) {
			...
			this.highlightCanvas = new Sprite("highlightCanvas");
			this.addChild(this.highlightCanvas);
			this.textOutCanvas = new Sprite("textOutCanvas");
			this.addChild(this.textOutCanvas);
			this.medianLineCanvas = new Sprite("medianLineCanvas");
			this.addChild(this.medianLineCanvas);
			if (this.isNameTextRequired()) {
				this.indicatorNameText = new TextField();
				this.indicatorNameText.selectable = false;
				this.indicatorNameText.textColor = IndicatorLayer.NAME_TEXT_COLOR;
				this.indicatorNameText.defaultTextFormat = new TextFormat(IndicatorLayer.INDICATOR_TEXT_FONT, IndicatorLayer.INDICATOR_TEXT_FONT_SIZE, IndicatorLayer.NAME_TEXT_COLOR);
			...
			for (let colorIndex = 0; colorIndex < IndicatorLayer.COLORS.length; colorIndex++) {
				const layerTextFormat = new TextField();
				layerTextFormat.selectable = false;
				layerTextFormat.textColor = this.getColor(colorIndex);
				layerTextFormat.defaultTextFormat = new TextFormat(IndicatorLayer.INDICATOR_TEXT_FONT, IndicatorLayer.INDICATOR_TEXT_FONT_SIZE, layerTextFormat.textColor);
		...
		private renderHistogramLine(dataSeriesArray: DataSeries[], dataSeriesIndex: number, param3: number, param4: number, viewPoint: ViewPoint, context: Context, detailLevel: Intervals, layerType: string) {
			...
			const yPos = this.getYPos(context, new IndicatorPoint(0, null as any as DataUnit));	// TODO
		...
		protected drawMedianLine(param1: number = -1, sprite: Sprite) {
			if (param1 === -1) {
				...
				if (!isNaN(value)) {
					...
					switch (this.getLineStyle(dataSeriesIndex)) {
						...
						case IndicatorLineStyle.HISTOGRAM_LINE:
							const _loc14_ = this.getYPos(context, new IndicatorPoint(0, null as any as DataUnit)); // TODO
							_loc13_ = this.getYPos(context, point);
