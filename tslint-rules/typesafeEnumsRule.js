...
var TypesafeEnumsWalker = /** @class */ (function (_super) {
    ...
    */
	TypesafeEnumsWalker.isNumber = function (type) {
		return (!!(type.flags & ts.TypeFlags.Number) || !!(type.flags & ts.TypeFlags.NumberLiteral)) && !(type.flags & ts.TypeFlags.EnumLiteral);
	};
	TypesafeEnumsWalker.isEnum = function (type) {
	...
	TypesafeEnumsWalker.prototype.checkTypes = function (node, nodeLeft, nodeRight) {
		var typeLeft = this.checker.getTypeAtLocation(nodeLeft);
		var typeRight = this.checker.getTypeAtLocation(nodeRight);
		//console.log("check", typeLeft, typeRight);
		var leftIsNumber = TypesafeEnumsWalker.isNumber(typeLeft);
		var rightIsNumber = TypesafeEnumsWalker.isNumber(typeRight);
		if (leftIsNumber) {
			if (rightIsNumber)
				return;
			if (TypesafeEnumsWalker.isEnum(typeRight))
				this.addFailure(this.createFailure(node.getStart(), node.getWidth(), Rule.FAILURE_STRING));
		}
		else if (rightIsNumber) {
			// !leftIsNumber
			if (TypesafeEnumsWalker.isEnum(typeLeft))
				this.addFailure(this.createFailure(node.getStart(), node.getWidth(), Rule.FAILURE_STRING));
		}
	};
